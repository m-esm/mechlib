...
- [x] SEV=med At 390×844 the install line clips at `github.com/m-esm/mecl` with no wrap or overflow cue; Copy is the only way to recover the URL. Fixed 2026-09-15: `#qs-install` wraps (`pre-wrap` + `overflow-wrap: anywhere`); measured `containsMechlib: true`, `h: 35`.
- [x] SEV=med At 390×844 the search placeholder clips (`descriptio`) and `press /` occupies the field on a phone that has no slash shortcut. Fixed 2026-09-15: hide `.search-hint` under 700px, bump input to 16px, swap placeholder to `Search parts…`. Desktop control restores the long placeholder and `press /`.
- [x] SEV=med The category chip row clips with no overflow hint: desktop cuts `Pumps & valves`, mobile cuts `Grippers & cla`. Fixed 2026-09-15: fade mask + 28px end padding on `.chip-scroll` so the last visible chip dissolves instead of hard-cutting a word.
